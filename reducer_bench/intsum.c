...
#include "../include/cilk/reducer.h"
#include "ktiming.h"


void identity_longsum(void* reducer, void* sum) {
  *((long*)sum) = 0;
}
...
CILK_C_DECLARE_REDUCER(long) my_int_sum_reducer = CILK_C_INIT_REDUCER(long,
    reduce_longsum, identity_longsum, __cilkrts_hyperobject_noop_destroy);
